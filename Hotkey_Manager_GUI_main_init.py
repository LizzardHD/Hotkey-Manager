...
class Hotkey_Manager:        
    def __init__(self,window):
        ...
        add_action_button = tk.Button(self.control_frame, text="Add Action", command=self.add_action)
        add_action_button.grid(row=1, column=0, columnspan=2)
        save_button = tk.Button(self.control_frame, text="Save Actions", command=self.save)
        save_button.grid(row=0, column=1)
        Load_button = tk.Button(self.control_frame, text="Load Actions", command=self.load)
        ...
        self.name_option = tk.OptionMenu(self.execution_frame, self.name_key, *execution_options, command=self.update_name_option)
    ...
    def add_action(self,action_type="Sleep", action_value=""):
        ...
        new_action_label = tk.Label(new_action_frame, text=f"{self.action_add_run_times:03}")
        ...
        new_delete_action_button = tk.Button(self.manager_frame, text="X", command=self.delete_action)
        new_delete_action_button.grid(row=4, columnspan=10, sticky="s")
        ...
        self.canvas_change(None)  # Update the canvas and action frame position
    ...
    def save(self):
        ...
        enter_button = tk.Button(self.save_window, text="Enter", command=self.save_action_set)
        enter_button.grid()
